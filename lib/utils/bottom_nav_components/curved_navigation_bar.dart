...
class CurvedNavigationBar extends StatefulWidget {
  ...
  final Color? activeColor; // Color for active icon & title

  CurvedNavigationBar({
    Key? key,
    required this.items,
    required this.titles, // Titles for each item
    this.index = 0,
    ...
    this.height = 75.0,
    this.maxWidth,
    this.activeColor, // Active color passed from theme
  })  : letIndexChange = letIndexChange ?? ((_) => true),
        assert(items.isNotEmpty),
        assert(0 <= index && index < items.length),
        assert(0 <= height && height <= 75.0),
        assert(maxWidth == null || 0 <= maxWidth),
        assert(items.length == titles.length), // Ensure titles match items
        super(key: key);
